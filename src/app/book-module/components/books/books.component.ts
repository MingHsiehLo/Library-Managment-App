...
export class BooksComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    const resolvedBooks: Book[] | string = this.activatedRoute.snapshot.data.resolveBooks;
    if (typeof resolvedBooks === 'string'){
      console.log('Home component book retrieve error.');
    } else {
      this.requestResult = resolvedBooks.sort(
        (a, b) => (a.title > b.title) ? 1 : (b.title > a.title) ? -1 : 0
      );
      this.books = this.requestResult;
    }
    this.collectionSize = this.requestResult.length;
    if (this.ISBN){
